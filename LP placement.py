...
def generate_grid(grid_size, objects, height, width, xpos, ypos, center):
    ...
    for y in range(grid_size):
        ...
        for x in range(grid_size):
            for obj in objects:
                if center:
                    if int(pulp.value(xpos[obj]) + width[obj] // 2) == x and int(pulp.value(ypos[obj]) + height[obj] // 2) == y:
                        grid[-1].append(1)
                        value_appended = True
                else:
                    if pulp.value(xpos[obj]) == x and pulp.value(ypos[obj]) == y:
                        ...
                        value_appended = True
                        break
                    elif pulp.value(xpos[obj])<= x <= pulp.value(xpos[obj])+width[obj]-1 and pulp.value(ypos[obj])<= y <= pulp.value(ypos[obj])+height[obj]-1:
                        grid[-1].append(1)
                        value_appended = True
                        break
...
def initiate_astar(grid, x, y, width, height, connections, center):
    ...
    for connect in connections:
        if center:
            start = (int(pulp.value(x[connect[0]]) + width[connect[0]]//2), int(pulp.value(y[connect[0]]) + height[connect[0]] //2))
            end = (int(pulp.value(x[connect[1]])+ width[connect[1]] //2) , int(pulp.value(y[connect[1]]) + height[connect[1]] //2))
            print(start, end)
        else:
...
def main():
    ...
    height = {'A': 10, 'B': 20, 'C': 40, 'D': 10, 'E': 4, 'F': 12, 'G': 5, 'H': 8}
    width = {'A': 20, 'B': 10, 'C': 20, 'D': 10, 'E': 10, 'F': 6, 'G': 10, 'H': 18}

    # space between objects

    x, y = linear_optimization_problem(objects, connections, grid_size, height, width, padding)
    grid = generate_grid(grid_size, objects, height, width, x, y, center)


    path = initiate_astar(grid, x, y, width, height, connections, center)
    cleaned_paths = simplify_all_paths(path)
    if clean_path:
        draw_result(grid_size, objects, cleaned_paths, height, width, x, y)
    else:
        draw_result(grid_size, objects, path, height, width, x, y)
