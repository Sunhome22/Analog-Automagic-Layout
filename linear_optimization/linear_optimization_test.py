...
class LinearOptimizationSolver:
    ...
    def __init__(self, object_info, connections, local_connections, grid_size, overlap_dict):
        ...
        # self.x_overlap = pulp.LpVariable.dicts("x_bin_ov", [(i, xv) for i in self.objects for xv in self.x_ov],
        #                                        cat="Binary")
        # self.y_overlap = pulp.LpVariable.dicts("y_bin_ov", [(i, yv) for i in self.objects for yv in self.y_ov],
        #                                        cat="Binary")
        self.coordinates_x = {}
        self.coordinates_y = {}
    ...
    def _constrain_mirror(self):
        for obj in self.mirrored_objects:
            ...
            self.problem_space +=self.coordinates_x[obj[0].number_id] == self.grid_size - self.coordinates_x[obj[1].number_id]
            ...
            for o2 in object_list:
                ...
                if o1 != o2 and [(o1,o2) ]not in self.mirrored_objects :
                    ...
                    if [o1,o2] in self.overlap_dict["top"] and [o1,o2] in self.overlap_dict["side"]:
                        self.problem_space += self.coordinates_x[o1] + self.width[
                            o1]  <= self.coordinates_x[
                                                  o2] + self.grid_size * (1 - z1), f"LeftOf_{o1}_{o2}"
                        self.problem_space += self.coordinates_x[o2] + self.width[
                            o2] <= self.coordinates_x[
                                                  o1] + self.grid_size * (1 - z2), f"RightOf_{o1}_{o2}"
                        self.problem_space += self.coordinates_y[o1] + self.height[
                            o1]  <= self.coordinates_y[
                                                  o2] + self.grid_size * (1 - z3), f"Below_{o1}_{o2}"
                        self.problem_space += self.coordinates_y[o2] + self.height[
                            o2]  <= self.coordinates_y[
                                                  o1] + self.grid_size * (1 - z4), f"Above_{o1}_{o2}"
                        ...
                        self.problem_space += self.coordinates_x[o2] + self.width[
                            o2] <= self.coordinates_x[
                                                  o1] + self.grid_size * (1 - z2), f"RightOf_{o1}_{o2}"
                        self.problem_space += self.coordinates_y[o1] + self.height[
                            o1] + self.OFFSET_Y * offset_y_enable <= self.coordinates_y[
                                                  o2] + self.grid_size * (1 - z3), f"Below_{o1}_{o2}"
                        self.problem_space += self.coordinates_y[o2] + self.height[
                            o2] + self.OFFSET_Y * offset_y_enable <= self.coordinates_y[
                                                  o1] + self.grid_size * (1 - z4), f"Above_{o1}_{o2}"

                    elif [o1,o2] in self.overlap_dict["top"]:
                        self.problem_space += self.coordinates_x[o1] + self.width[
                            o1] + self.OFFSET_X  <= self.coordinates_x[
                                                  o2] + self.grid_size * (1 - z1), f"LeftOf_{o1}_{o2}"
                        self.problem_space += self.coordinates_x[o2] + self.width[
                            o2] + self.OFFSET_X  <= self.coordinates_x[
                                                  o1] + self.grid_size * (1 - z2), f"RightOf_{o1}_{o2}"
                        self.problem_space += self.coordinates_y[o1] + self.height[
                            o1] <= self.coordinates_y[
                                                  o2] + self.grid_size * (1 - z3), f"Below_{o1}_{o2}"
                        ...
                        self.problem_space += self.coordinates_x[o1] + self.width[o1] +self.OFFSET_X*offset_x_enable <= self.coordinates_x[
                            o2] + self.grid_size * (1 - z1), f"LeftOf_{o1}_{o2}"
                        self.problem_space += self.coordinates_x[o2] + self.width[o2] +self.OFFSET_X*offset_x_enable <= self.coordinates_x[
                            o1] + self.grid_size * (1 - z2), f"RightOf_{o1}_{o2}"
                        self.problem_space += self.coordinates_y[o1] + self.height[o1]+self.OFFSET_Y*offset_y_enable <= self.coordinates_y[
                            o2] + self.grid_size * (1 - z3), f"Below_{o1}_{o2}"
                        self.problem_space += self.coordinates_y[o2] + self.height[o2]+self.OFFSET_Y*offset_y_enable <= self.coordinates_y[
                            o1] + self.grid_size * (1 - z4), f"Above_{o1}_{o2}"
    ...
    def _solve_linear_optimization_problem(self):
       # self.problem_space += pulp.lpSum(
        ...
        self.problem_space += pulp.lpSum(
            [self.d_x[(o1.starting_comp, o1.end_comp)] + self.d_y[(o1.starting_comp, o1.end_comp)] for o1 in
            self.connections.values()])*000.1 +((self.x_max - self.x_min) +(self.y_max-self.y_min)) * 1000 , "totalWireLength"
