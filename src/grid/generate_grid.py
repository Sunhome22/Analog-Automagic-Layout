...
class GridGeneration:
        ...
        for obj in self.components:
            ...
            if not check_instance(obj):
                for port in obj.layout_ports:
                    ...
                    x1 = (obj.transform_matrix.c + (port.area.x1 + port.area.x2)/2 - self.used_area.x1
                                                 + self.GRID_LEEWAY_X)/self.SCALE_FACTOR
                    y1 = (obj.transform_matrix.f + (port.area.y1 + port.area.y2)/2 - self.used_area.y1
                                                 + self.GRID_LEEWAY_Y)/self.SCALE_FACTOR

                    frac_x, int_x = math.modf(x1)
                    frac_y, int_y = math.modf(y1)
                    self.port_area.setdefault(str(obj.number_id)+obj.type + port.type, Coordinates(x=round(int_x),
                                                                                                   y=round(int_y)))

                    self.port_coordinates.setdefault(str(obj.number_id)+obj.type + port.type,
                                                     Coordinates(x=int(obj.transform_matrix.c
                                                                 + (port.area.x1 + port.area.x2)/2),
                                                                 y=int(obj.transform_matrix.f
                                                                 + (port.area.y1 + port.area.y2)/2)))
                    self.scaled_port_coordinates.setdefault(str(obj.number_id)+obj.type+port.type,
                                                            Coordinates(x=int(int_x), y=int(int_y)))
    ...
    def __generate_grid(self):
        ...
        scaled_grid_size_y = list(math.modf((self.used_area.y2-self.used_area.y1+2*self.GRID_LEEWAY_Y)
                                            / self.SCALE_FACTOR))
        scaled_grid_size_x = list(math.modf((self.used_area.x2 - self.used_area.x1 + 2 * self.GRID_LEEWAY_X)
                                            / self.SCALE_FACTOR))
...
def check_ignorable_port(logger, components, object_id, port):
    for obj in components:
        if obj.number_id == int(object_id):
            ...
            return (re.search(".*VSS.*", port_connection, re.IGNORECASE) or
                    re.search(".*VDD.*", port_connection, re.IGNORECASE))
