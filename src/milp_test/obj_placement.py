...
class ComponentPlacementEnvironment(gym.Env):
    def __init__(self, grid_size, component_size, components_total, max_steps):
        ...
        self.action_space = gym.spaces.MultiDiscrete(
            [components_total, 5, 5]  # component_index, dx (-1, 0, 1), dy (-1, 0, 1)
        )
        self.reset()
    ...
    def step(self, action):
        ...
        self.component_positions[component_index, 0] = np.clip(
            self.component_positions[component_index, 0] + mx[dx], 0, self.grid_size - self.component_size[0]
        )
        self.component_positions[component_index, 1] = np.clip(
            self.component_positions[component_index, 1] + mx[dy], 0, self.grid_size - self.component_size[1]
        )
    ...
    def reward(self):
        ...
        for i in range(self.components_total):
            for j in range(i + 1, self.components_total):
                dist =  abs(self.component_positions[i][0] - self.component_positions[j][0]) + abs(self.component_positions[i][1] - self.component_positions[j][1])
                if dist <=2:
                    reward += 5
                total_distance += dist
        ...
        if self.steps == self.max_steps-1:
            print("this is the penultimate step")
            print(self.component_positions)

        print(total_distance)
        if total_distance < 8:
            reward += 100
        if total_distance >= self.previous_total:
            reward += -total_distance
        if total_distance < self.previous_total or total_distance == 0:
            reward += 50

        self.previous_total = total_distance
...
def object_placement():
    ...
    model = PPO("MlpPolicy", environment, verbose=1, device="cpu", n_steps =1024, batch_size = 64, learning_rate=1e-3, ent_coef=0.1) #
